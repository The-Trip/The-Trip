...
function FlightResults({
  ...
  function handleClick(
    ...
    let flightDetailsObject = {
      ...
      returnLocalArrivalTime: `${addZero(
        arrivalTimeReturn.getHours()
      )}:${addZero(arrivalTimeReturn.getMinutes())}:00`,
      price: flightDetail.price
    };

    addFlightToDB(flightDetailsObject).then(() => {
  ...
  return (
    <div>
      ...
      {flightResults.map(function(flightDetail) {
        let id = flightDetail.route[0].combination_id;
        ...
        return (
          <article key={id} className="results">
